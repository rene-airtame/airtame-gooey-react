...
describe('<Collapser />', () => {
  ...
  it('should initialize closed with collapsed prop set to true', () => {
    const wrapper = shallow(
      <Collapser title="Test" collapsed>
        content
      </Collapser>
    );
  ...
  it('should toggle from open to close and viceversa', () => {
    ...
    expect(wrapper.find('.gooey-collapser__container')).not.toHaveClassName(
      'gooey-collapser__container--expanded'
    );

    wrapper.setState({ collapsed: false });
    expect(wrapper.find('.gooey-collapser__container')).toHaveClassName(
      'gooey-collapser__container--expanded'
    );
  });
